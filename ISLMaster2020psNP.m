function X = ISLMaster2020psNP(pAR,L,N,NN,ro,rr,delta,t,ST,ps)
rng('shuffle')
%Function for simulating the Mixed algorithms with agent probability of
%switching stratey per timestep ps/pi, i.e. non-constant agent evader/follower status.
...
%pAR=0.5;%proportion of non-followers 
%ST=Strategy Lead (-1), Mixed (0), HERD (+1)
%ps=probability of agent switching strategy per timestep
...
%     end
  ...
  %IF nearest agent follow Lead, else herd.
  ...
  if k>1
    ...
    P=ISLObjects2020ps(P,NN,R,ro,rr,delta,ps); %Update Position of objects
