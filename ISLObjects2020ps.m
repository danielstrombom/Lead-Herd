function PP = ISLObjects2020ps(P,NN,R,ro,rr,delta,ps)
rng('shuffle')
%Function to simulate objects attracted/repelled to/from shepherd with 
%probability of switching strategy per timestep ps/pi
...
for i=1:N % Go through every object.
    ...
    %Potentially switch strategy in this time step
    
    if rand<ps %If particle i will switch
        if PP(i,6)==1 %If follower
            PP(i,6)=-1; %become evader
        else
            PP(i,6)=1;
        end
    end
